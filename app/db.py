...
class SearchRun(Base):
    ...
    current_stage = Column(String, nullable=True)  # planning, harvesting, screening, appraisal, reporting, completed
    stage_progress = Column(Float, nullable=True, default=0.0)  # 0.0 to 1.0
    total_records = Column(Integer, nullable=True, default=0)  # For progress estimation
    processed_records = Column(Integer, nullable=True, default=0)  # Records processed in current stage
...
class Record(Base):
    ...
    institution = Column(String, nullable=True)  # Institution/organization associated with the research
...
def _ensure_sqlite_schema_compat() -> None:
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            add_if_missing("institution", "VARCHAR")
            ...
            # Ensure appraisals.content_type and content_source exist
            if "content_type" not in app_cols:
                try:
                    conn.exec_driver_sql("ALTER TABLE appraisals ADD COLUMN content_type VARCHAR")
                except Exception:
                    pass
            if "content_source" not in app_cols:
                try:
                    conn.exec_driver_sql("ALTER TABLE appraisals ADD COLUMN content_source VARCHAR")
                except Exception:
                    pass
    except Exception:
