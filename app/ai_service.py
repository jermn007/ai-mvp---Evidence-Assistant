...
def extract_json_from_markdown(content: str) -> str:
    """
    Extract JSON content from a string that may contain markdown code blocks.

    Args:
        content: Raw content string that may contain ```json...``` or ```...``` markers
...
Please generate a clear, focused research question that incorporates these components appropriately for a systematic literature review."""
    ...
    async def extract_lico_from_question(self, question: str) -> Optional[LICO]:
        ...
        system_prompt = """You are an expert systematic review methodologist specializing in extracting LICO components.

LICO Framework Definition:
- Learner: The target population, participants, subjects, or patients (who is being studied?)
- Intervention: What is being studied, implemented, tested, or compared (what is being done or examined?)
- Context: The setting, environment, conditions, or circumstances (where or under what conditions?)
- Outcome: The measured results, effects, endpoints, or findings (what results are being measured?)

Instructions:
1. Carefully analyze the research question
2. Extract each component that is explicitly stated or clearly implied
3. Be specific - extract actual phrases from the question when possible
4. If a component is truly not mentioned, leave it as an empty string
5. Return valid JSON only - no markdown formatting or code blocks"""

        human_prompt = f"""Extract LICO components from this research question:

"{question}"

Examples to guide extraction:
- Question: "What is the effect of simulation training on medical students' clinical skills in emergency departments?"
  {{
    "learner": "medical students",
    "intervention": "simulation training",
    "context": "emergency departments",
    "outcome": "clinical skills"
  }}

- Question: "What is the median survival time and associated prognostic factors for 40-year-old patients diagnosed with stage 4 liver disease, as reported in the peer-reviewed literature?"
  {{
    "learner": "40-year-old patients diagnosed with stage 4 liver disease",
    "intervention": "prognostic factors",
    "context": "peer-reviewed literature",
    "outcome": "median survival time"
  }}

Now extract from the provided question above. Return ONLY a JSON object with these exact keys: learner, intervention, context, outcome"""
        ...
        try:
            ...
            logger.error(f"Error extracting LICO from question: {e}", exc_info=True)
            return None
