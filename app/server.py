...
from app.models_extended import AppraisalMethodology, MultiMethodAppraisal
...
SearchRun = getattr(_db, "SearchRun")
...
class RunRequest(BaseModel):
    ...
    appraisal_profile: str | None = None     # e.g., "comprehensive", "medical_education", "traditional"
...
async def run(req: RunRequest):
    try:
        ...
        # Set appraisal profile if specified
        if req.appraisal_profile:
            init_state["appraisal_profile"] = req.appraisal_profile
...
class RunWithPressBody(BaseModel):
    ...
    appraisal_profile: Optional[str] = None  # e.g., "comprehensive", "medical_education", "traditional"
...
async def run_with_press(body: RunWithPressBody):
    try:
        ...
        if body.appraisal_profile:
            init_state["appraisal_profile"] = body.appraisal_profile
        # Generate run_id immediately so we can return it to the client
        from uuid import uuid4 as generate_uuid
        run_id = str(generate_uuid())
        init_state["run_id"] = run_id

        logger.info(f"run_with_press: starting graph execution with query: {generic_q[:100]}...")
        logger.info(f"run_with_press: thread_id={tid}, run_id={run_id}, sources={srcs}")

        # Execute workflow in background task so we can return run_id immediately
        import asyncio

        async def execute_workflow_background():
            try:
                final = await GRAPH.ainvoke(init_state, config={"configurable": {"thread_id": tid}})
                logger.info(f"run_with_press: graph execution completed successfully for run_id={run_id}")

                try:
                    persisted_id = persist_run(final)
                    logger.info(f"run_with_press: run persisted with id: {persisted_id}")

                    # Mark workflow as completed
                    from app.persist import update_run_progress
                    update_run_progress(run_id, current_stage="completed", stage_progress=1.0)
                    logger.info(f"run_with_press: run {run_id} marked as completed")
        ...
        # Start the background task and keep a reference to prevent garbage collection
        task = asyncio.create_task(execute_workflow_background())
        background_tasks.add(task)
        task.add_done_callback(background_tasks.discard)

        # Return immediately with run_id so client can start polling for progress
        return {
            "thread_id": tid,
            "run_id": run_id,
            "status": "running",
...
async def get_run_status(run_id: str):
    """Get current execution status and progress for a running workflow"""
    stmt = SAselect(SearchRun).where(SearchRun.id == run_id)
    runs = await _select_stmt(stmt)
    ...
    # Determine if run is completed by checking if we have records and appraisals
    ...
    current_stage = getattr(run, "current_stage", None) or "completed"
    stage_progress = getattr(run, "stage_progress", 1.0) or 1.0
    total_records = getattr(run, "total_records", len(records))
    processed_records = getattr(run, "processed_records", len(appraisals))

    # If we have appraisals equal to records, mark as completed
    if len(appraisals) > 0 and len(appraisals) >= len(records) and current_stage != "completed":
    ...
    return {
        ...
        "appraisals_count": len(appraisals),
        "is_completed": current_stage == "completed",
        "created_at": run.created_at.isoformat() if run.created_at else None
    }

@app.get("/runs.page.json")
async def list_runs(limit: int = 20, offset: int = 0):
    """List all search runs with pagination"""
    try:
        init_db()
        with get_session() as s:
            # Get total count
            count_stmt = SAselect(func.count()).select_from(SearchRun)
            ...
            runs = list(s.scalars(runs_stmt))

            # For each run, get record count and appraisal label counts
            items = []
            for run in runs:
                rec_count_stmt = SAselect(func.count()).select_from(Record).where(Record.run_id == run.id)
                n_records = s.scalar(rec_count_stmt) or 0

                app_stmt = SAselect(Appraisal).where(Appraisal.run_id == run.id)
                appraisals = list(s.scalars(app_stmt))
                ...
                for app in appraisals:
                    ...
                    if rating:
                        label_counts[rating] = label_counts.get(rating, 0) + 1
                ...
                items.append({
                    ...
                    "created_at": run.created_at.isoformat() if run.created_at else None,
                    "n_records": n_records,
                    "n_appraised": len(appraisals),
                    "label_counts": label_counts if label_counts else None,
                    "current_stage": run.current_stage,
                    "is_completed": run.current_stage == "completed" if run.current_stage else None
                })
        ...
        logger.error(f"list_runs failed: {type(e).__name__}: {e}")
...
@app.get("/runs/{run_id}/records/{record_id}/multi-method-appraisals.json")
def get_multi_method_appraisals(run_id: str, record_id: str):
    """Return multi-method appraisal results for a specific record within a run."""
    try:
        with get_session() as session:
            if record_id.startswith(f"{run_id}:"):
                db_record_id = record_id
            else:
                db_record_id = f"{run_id}:{record_id}"

            rows = (
                session.query(MultiMethodAppraisal, AppraisalMethodology)
                .join(AppraisalMethodology, MultiMethodAppraisal.methodology_id == AppraisalMethodology.id)
                .filter(
                    MultiMethodAppraisal.run_id == run_id,
                    MultiMethodAppraisal.record_id == db_record_id,
                )
                .order_by(AppraisalMethodology.name)
                .all()
            )

            appraisals = []
            for mma, methodology in rows:
                appraisals.append({
                    "id": mma.id,
                    ...
                    "methodology": {
                        ...
                        "method_type": methodology.method_type.value if methodology.method_type else None,
                        "criteria": methodology.criteria,
                        "scoring_config": methodology.scoring_config,
                        "requires_full_text": methodology.requires_full_text,
                    },
                    "scores": mma.scores,
                    "overall_score": mma.overall_score,
                    "rating": mma.rating,
                    "rationale": mma.rationale,
                    "evidence_citations": mma.evidence_citations_list,
                    "confidence": mma.confidence,
                    "used_full_text": mma.used_full_text,
                    "sections_analyzed": mma.sections_analyzed,
                    "assessment_time": mma.assessment_time,
                    "created_at": mma.created_at.isoformat() if mma.created_at else None,
                })
        ...
        logger.error("get_multi_method_appraisals failed: %s", e)
        raise HTTPException(status_code=500, detail=f"multi-method appraisals retrieval failed: {type(e).__name__}: {e}")
