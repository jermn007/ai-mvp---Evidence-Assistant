...
async def synthesize_research(
    ...
    # Analyze multi-method appraisal insights
    multi_method_insights = _analyze_multi_method_insights(findings)

    synthesis_prompt = f"""As an expert instructional design researcher and methodological specialist, synthesize the following literature to answer the research question comprehensively using advanced appraisal methodologies.
...
FULL-TEXT AVAILABILITY & PROCESSING:
...
Full-text analysis provides enhanced evidence extraction, contextual understanding, and validation of abstract-only insights.

MULTI-METHOD APPRAISAL INSIGHTS:
{multi_method_insights}
...
Provide a comprehensive research synthesis that leverages multi-method appraisal frameworks (Kirkpatrick, Towle, RAG Quality) and full-text analysis:
...
"""
    ...
    try:
        ...
        synthesis_result = _parse_synthesis_response(response.content, findings, all_supporting_quotes)

        # Enhance with quality metrics from multi-method appraisal
        synthesis_result = _enhance_with_quality_metrics(synthesis_result, findings)
...
def _build_citation_map(findings: List[StudyFindings]) -> Dict[str, CitationMetadata]:
    ...
    for idx, finding in enumerate(findings):
        ...
        citations[key] = CitationMetadata(
            citation_key=key,
            title=finding.title,
            authors=finding.authors or "Unknown Authors",
...
def _analyze_multi_method_insights(findings: List[StudyFindings]) -> str:
    """
    Analyze and summarize multi-method appraisal insights across studies.
    Provides context on Kirkpatrick levels, Towle patient involvement, and quality assessments.
    """
    insights = []

    # Check for multi-method metadata
    kirkpatrick_data = []
    towle_data = []
    full_text_data = []

    for finding in findings:
        # Extract multi-method information if available
        if hasattr(finding, 'metadata') and finding.metadata:
            metadata = finding.metadata

            if 'kirkpatrick_level' in metadata:
                kirkpatrick_data.append({
                    'title': finding.title[:40] + "...",
                    'level': metadata.get('kirkpatrick_level', 'Unknown'),
                    'score': metadata.get('kirkpatrick_score', 0)
                })

            if 'towle_level' in metadata:
                towle_data.append({
                    'title': finding.title[:40] + "...",
                    'level': metadata.get('towle_level', 'Unknown'),
                    'score': metadata.get('towle_score', 0)
                })
        ...
        if finding.full_text_content and finding.full_text_content.extraction_success:
            full_text_data.append({
                'title': finding.title[:40] + "...",
                'word_count': finding.full_text_content.word_count,
                'confidence': finding.full_text_content.confidence
            })

    # Generate insights
    if kirkpatrick_data:
        levels = [k['level'] for k in kirkpatrick_data]
        avg_score = sum(k['score'] for k in kirkpatrick_data) / len(kirkpatrick_data)
        insights.append(f"Kirkpatrick evaluation framework applied to {len(kirkpatrick_data)} studies. "
                       f"Average score: {avg_score:.2f}. Levels represented: {', '.join(set(levels))}")

    if towle_data:
        levels = [t['level'] for t in towle_data]
        avg_score = sum(t['score'] for t in towle_data) / len(towle_data)
        insights.append(f"Towle patient involvement assessment applied to {len(towle_data)} studies. "
                       f"Average score: {avg_score:.2f}. Involvement levels: {', '.join(set(levels))}")

    if full_text_data:
        avg_words = sum(f['word_count'] for f in full_text_data) / len(full_text_data)
        avg_confidence = sum(f['confidence'] for f in full_text_data) / len(full_text_data)
        insights.append(f"Full-text analysis completed for {len(full_text_data)} studies. "
                       f"Average document length: {avg_words:.0f} words with {avg_confidence:.1%} extraction confidence")

    if not insights:
        return "Standard single-method RAG Quality assessment applied. Multi-method analysis (Kirkpatrick, Towle) not available for this synthesis."

    return "MULTI-METHOD APPRAISAL: " + " ".join(insights)


def _enhance_with_quality_metrics(synthesis: ResearchSynthesis, findings: List[StudyFindings]) -> ResearchSynthesis:
    """
    Enhance synthesis with quality metrics from multi-method appraisal results.
    Incorporates Kirkpatrick, Towle, and full-text analysis insights.
    """
    try:
        # Collect multi-method quality insights
        quality_insights = []

        # Full-text availability impact
        full_text_studies = [f for f in findings if f.full_text_content and f.full_text_content.extraction_success]
        if full_text_studies:
            full_text_percent = len(full_text_studies) / len(findings) * 100
            quality_insights.append(
                f"Full-text analysis available for {len(full_text_studies)}/{len(findings)} studies "
                f"({full_text_percent:.0f}%), providing enhanced evidence extraction and validation."
            )

        # Kirkpatrick-level insights (if available in metadata)
        kirkpatrick_insights = []
        towle_insights = []

        for finding in findings[:5]:  # Top 5 studies for quality insights
            # Check if study has multi-method appraisal metadata
            if hasattr(finding, 'metadata') and finding.metadata:
                metadata = finding.metadata

                # Extract Kirkpatrick insights
                if 'kirkpatrick_score' in metadata:
                    kirkpatrick_insights.append({
                        'title': finding.title[:50] + "...",
                        'level': metadata.get('kirkpatrick_level', 'Unknown'),
                        'score': metadata.get('kirkpatrick_score', 0),
                        'evidence': metadata.get('kirkpatrick_evidence', '')
                    })

                # Extract Towle insights
                if 'towle_score' in metadata:
                    towle_insights.append({
                        'title': finding.title[:50] + "...",
                        'level': metadata.get('towle_level', 'Unknown'),
                        'score': metadata.get('towle_score', 0),
                        'evidence': metadata.get('towle_evidence', '')
                    })

        # Enhance methodological quality assessment
        enhanced_quality = synthesis.methodological_quality

        if kirkpatrick_insights:
            avg_kirkpatrick = sum(k['score'] for k in kirkpatrick_insights) / len(kirkpatrick_insights)
            enhanced_quality += f" Multi-level evaluation framework analysis shows average Kirkpatrick score of {avg_kirkpatrick:.2f}."

        if towle_insights:
            avg_towle = sum(t['score'] for t in towle_insights) / len(towle_insights)
            enhanced_quality += f" Patient involvement assessment reveals average Towle score of {avg_towle:.2f}."

        if quality_insights:
            enhanced_quality += " " + " ".join(quality_insights)

        # Enhance future research directions with methodology-specific recommendations
        enhanced_future_research = synthesis.future_research_directions.copy()

        if kirkpatrick_insights:
            kirkpatrick_levels = [k['level'] for k in kirkpatrick_insights]
            if 'L4' not in kirkpatrick_levels:
                enhanced_future_research.append(
                    "Conduct studies measuring organizational impact (Kirkpatrick L4) to strengthen evidence of long-term effectiveness."
                )
            if 'L3' not in kirkpatrick_levels:
                enhanced_future_research.append(
                    "Include behavioral change measurement (Kirkpatrick L3) to assess real-world application of learning."
                )

        if towle_insights:
            towle_levels = [t['level'] for t in towle_insights]
            high_involvement = any('5' in level or '6' in level for level in towle_levels)
            if not high_involvement:
                enhanced_future_research.append(
                    "Explore higher levels of patient involvement (Towle L5-L6) to enhance medical education authenticity and impact."
                )

        # Enhance key recommendations with specific quality insights
        enhanced_recommendations = synthesis.key_recommendations.copy()

        if full_text_studies:
            enhanced_recommendations.append(
                "Prioritize full-text document analysis in systematic reviews to capture nuanced implementation details and contextual factors."
            )

        if kirkpatrick_insights or towle_insights:
            enhanced_recommendations.append(
                "Apply multi-method appraisal frameworks (Kirkpatrick, Towle, RAG Quality) to comprehensively evaluate educational intervention effectiveness."
            )

        # Create enhanced synthesis
        return ResearchSynthesis(
            executive_summary=synthesis.executive_summary,
            research_question_answer=synthesis.research_question_answer,
            lico_insights=synthesis.lico_insights,
            evidence_strength=synthesis.evidence_strength,
            confidence_level=synthesis.confidence_level,
            key_recommendations=enhanced_recommendations[:5],  # Limit to top 5
            knowledge_gaps=synthesis.knowledge_gaps,
            methodological_quality=enhanced_quality,
            future_research_directions=enhanced_future_research[:4],  # Limit to top 4
            supporting_evidence=synthesis.supporting_evidence,
            full_text_availability=synthesis.full_text_availability,
            study_citations=synthesis.study_citations,
        )

    except Exception as e:
        logger.warning(f"Failed to enhance synthesis with quality metrics: {e}")
        return synthesis  # Return original if enhancement fails
